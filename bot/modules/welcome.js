...
exports.onUserJoin = function(bot) {
  bot.on('guildMemberAdd', async member => {
    member.send({
      embed: {
        ...
        description:
          ...
          'To see more info about LBRY and the Discord, Follow these instructions:\n' +
          'React with :angry: to see the Discord Ground Rules - \n' +
          '**You accept these rules by continuing, whether you viewed them or not is on you!**\n' +
          'React with :thinking: to see LBRY FAQ\n' +
          'React with :thumbsup: to see more info on Spee.ch\n' +
          'React with :nerd: to see some Helpful Hints',
    ...
    m.react(speech);
    m.react(hints);
    const reactions = await member.awaitReactions(reaction => reaction.emoji.name === rules || reaction.emoji.name === faq || reaction.emoji.name === speech || reaction.emoji.name === hints, { time: 10000 });
    if (reactions.get(rules).count > 1) {
      member.send({
        embed: {
          description:
    ...
    if (reactions.get(speech).count > 1) {
      member.send({
        embed: {
    ...
    if (reactions.get(hints).count > 1) {
      member.send({
        embed: {
          description:
...
exports.welcome = {
  ...
  process: async function(bot, msg, suffix) {
    ...
    const m = await msg.mentions.members.first().send({
      embed: {
        title: '*Click here for more info about LBRY!*',
        description:
          '**Welcome to LBRY Discord Community, you are now officially a LBRYian!** \n' +
    ...
    m.react(speech);
    m.react(hints);
    const reactions = await m.awaitReactions(reaction => reaction.emoji.name === rules || reaction.emoji.name === faq || reaction.emoji.name === speech || reaction.emoji.name === hints, { time: 10000 });
    if (reactions.get(rules).count > 1) {
